"""批量调用 patched pycdc 反编译所有 v2 .pyc 文件。
...
IN_PYC_DIR = Path(os.environ.get("IN_PYC_DIR", "/mnt/e/星际猎人/dumped/modules_all_translated_v2"))
OUT_PY_DIR = Path(os.environ.get("OUT_PY_DIR", "/mnt/e/星际猎人/dumped/modules_all_decompiled_v2"))
...
def main():
    ...
    print(f"  ok (non-empty output): {counts['ok']} ({100*counts['ok']/len(files):.1f}%)", file=sys.stderr)
    print(f"  empty: {counts['empty']}", file=sys.stderr)
