"""dump科技/模块配置表：补齐 techId→EFFECT 映射链路所需的全部表

链路：techId(战报) → Tb_cfg_tech_blueprint.MAIN_SKILL → Tb_cfg_tech_bp_skill
      → Tb_cfg_tech_bp_skill_level.EFFECTS → {effect_id, param}

输出到 data/client/config/
"""
import frida, time, json, os

OUTDIR = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "config").replace("\\", "/")
OUTDIR_PY = json.dumps(OUTDIR)  # 合法 Python 字符串字面量，如 "E:/战斗模拟器/..."
...
try {
    ...
    var pyCode = [
        ...
        "    dump_table('common.config.db.Tb_cfg_weapon_tech_recycle', 'cfg_weapon_tech_recycle.json')",
        "    dump_table('common.config.db.Tb_cfg_module', 'cfg_module.json')",
        "    dump_table('common.config.db.Tb_cfg_tech_blueprint', 'cfg_tech_blueprint.json')",
        "    dump_table('common.config.db.Tb_cfg_tech_bp_skill', 'cfg_tech_bp_skill.json')",
        "    dump_table('common.config.db.Tb_cfg_tech_bp_skill_level', 'cfg_tech_bp_skill_level.json')",
        "    dump_table('common.config.db.Tb_cfg_tech_store', 'cfg_tech_store.json')",
        "except Exception:",
