...
try {
    ...
    var pyCode = [
        ...
        "    attrs=[a for a in dir(bu) if not a.startswith('__')]",
        "    out.append('blueprint_utils全部: '+str(attrs))",
        "    # 找scheme/code相关",
        "    for a in attrs:",
        "        if any(k in a.lower() for k in ['scheme','code','encode','decode','share','pack']):",
        "            obj=getattr(bu,a)",
        "            out.append(a+': type='+str(type(obj)))",
        "            if callable(obj):",
        "                out.append('  (callable)')",
        "except Exception:",
