"""探测 enhance 记录的完整字段（含可能的隐藏等级数值字段）"""
...
try {
    ...
    var pyCode = [
        ...
        "    candidates=[n for n in dir(db) if any(k in n.lower() for k in ['enhance','adjust','intensify'])]",
        "    out.append('candidates: '+str(candidates))",
        ...
        "    if rec is not None:",
        "        out.append('rec type='+str(type(rec)))",
        "        if hasattr(rec,'keys'):",
        "            out.append('keys='+str(list(rec.keys())))",
        "        else:",
        "            out.append('repr='+repr(rec)[:500])",
        "    else:",
        "        out.append('405010105 not found')",
        "except Exception:",
