...
test('CV3000 未实现的 EFFECT 透明记录（不报错）', async () => {
  const store = await getStore();
  const bp = resolveBlueprint(store, '80201', CV3000_TECH_STR);
  // CV3000 还装了自维修(12270)、舰队集火(12263)、战略打击(2065)、装甲(10033)等
  // 这些 EFFECT_ID 暂未实现，应进 unresolved[]
  assert.ok(bp.unresolved.length > 0, '应有未实现效果记录');

  // 确认 unresolved 里有已知 EFFECT_ID
  const unresolvedIds = new Set(bp.unresolved.map((u) => u.effectId));
  assert.ok(unresolvedIds.has(12270), '应记录自维修(12270)');
  assert.ok(unresolvedIds.has(10033), '应记录装甲强化(10033)');
});
