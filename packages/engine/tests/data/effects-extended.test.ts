...
test('CV3000 全部EFFECT已归类收录（0 unresolved）', async () => {
  const store = await getStore();
  const tech =
  ...
  // 全部EFFECT_ID已分类收录，unresolved为空
  assert.equal(bp.unresolved.length, 0, '0 unresolved');
  // 验证分类：应有 defense/flagship/skill 等类别
  const categories = new Set(bp.effects.map((e) => e.category));
  assert.ok(categories.has('flagship'), '应有旗舰类');
  assert.equal(bp.hasAutoRepair, true, 'CV3000应有自维修');
