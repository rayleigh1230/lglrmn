...
import { resolveShipWeapons, computeFirepower } from '../../src/data/blueprintCalc.js';
...
const fg = computeFirepower(fgWeapons);
console.log(`  对舰=${fg.antiShip} 防空=${fg.antiAir} 攻城=${fg.siege}`);
assert(Math.abs(fg.antiShip - 1543) <= 1, `FG300对舰≈1543 (实际${fg.antiShip})`);
assert(Math.abs(fg.antiAir - 1851) <= 1, `FG300防空≈1851 (实际${fg.antiAir})`);
...
function makeW(partial: Partial<W>): W {
  return {
    weaponId: '', systemId: '', systemLabel: '',
    dph: 0, attackRounds: 1, attackCount: 1, installNum: 1, antiaircraftRatio: 0,
    shotsPerCycle: 1, fireDuration: 0, cooldown: 0, weaponType: 0, specialTargetLogic: 0,
    destroyCoef: 0, aircraftCoef: 0, airBaseBonus: 0, airCdReduction: 0, airDurReduction: 0,
    isAirborne: false, category: 'antiShip' as const,
    ...partial,
  };
}

// FG300主炮13011: ship=1542 air=1851 siege=324 (action=(3,2,30,1,3000), install=3, DC=14, cd=4, airBaseBonus=50)
const w13011 = makeW({ dph: 30, attackRounds: 3, attackCount: 1, installNum: 3, fireDuration: 3, cooldown: 4, destroyCoef: 14, antiaircraftRatio: 200, airBaseBonus: 50 });
const fp13011 = computeFirepower([w13011]);
console.log(`  13011: 对舰=${fp13011.antiShip} 防空=${fp13011.antiAir} 攻城=${fp13011.siege}`);
assert(Math.abs(fp13011.antiShip - 1543) <= 1, `13011对舰≈1543 (实际${fp13011.antiShip})`);
assert(Math.abs(fp13011.antiAir - 1851) <= 1, `13011防空≈1851 (实际${fp13011.antiAir})`);
...
const w16031 = makeW({ dph: 700, attackRounds: 1, attackCount: 2, installNum: 2, fireDuration: 0, cooldown: 16, destroyCoef: 32 });
const fp16031 = computeFirepower([w16031]);
...
const w15017 = makeW({ dph: 40, attackRounds: 1, attackCount: 1, installNum: 4, fireDuration: 0, cooldown: 4, destroyCoef: 3 });
const fp15017 = computeFirepower([w15017]);
...
const w16033 = makeW({ dph: 280, attackRounds: 1, attackCount: 2, installNum: 3, fireDuration: 0, cooldown: 16, destroyCoef: 18 });
const fp16033 = computeFirepower([w16033]);
...
const w13124 = makeW({ dph: 20, attackRounds: 4, attackCount: 1, installNum: 2, fireDuration: 4, cooldown: 6, antiaircraftRatio: 150, airBaseBonus: 60, airCdReduction: 40, airDurReduction: 40 });
const fp13124 = computeFirepower([w13124]);
...
const w13122 = makeW({ dph: 15, attackRounds: 3, attackCount: 1, installNum: 2, fireDuration: 3, cooldown: 3, antiaircraftRatio: 150, airBaseBonus: 10, airCdReduction: 10, airDurReduction: 10 });
const fp13122 = computeFirepower([w13122]);
